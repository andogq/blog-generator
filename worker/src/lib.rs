...
pub async fn main(mut req: Request, env: Env, _ctx: worker::Context) -> Result<Response> {
    ...
    if path.next().map(|base| base == "_").unwrap_or(false) {
        ...
        if let (Ok(domains_store), Ok(Some(domain))) = (
            ...
            if let Ok(Some(user_token)) = domains_store.get(&domain).text().await {
                let mut user_token = user_token.split(',');
                if let (Some(user), Some(token)) = (user_token.next(), user_token.next()) {
                    ...
                    Response::error("Invalid user and token", 500)
                }
            } else {
